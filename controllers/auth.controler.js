...
const BlacklistedToken = require("../models/model.blacklist");
...
const logout = async (req, res) => {
  try {
    ...
    if (!token) {
      return res.status(400).json({ message: "Token is missing or not provided" });
    }
  ...
  try {
    ...
    const mailOptions = {
      ...
      html: `
        ...
        <h3>${req.protocol}://${req.get(
        "host"
      )}/users/reset-password/${token}</h3>
        <a href="${req.protocol}://${req.get(
        "host"
      )}/users/reset-password/${token}">Reset Password</a>
